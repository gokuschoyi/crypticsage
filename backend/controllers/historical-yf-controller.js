...
const initialSaveHistoricalDataYFinance = async (req, res) => {
    const tickersList = ["AAPL", "TSLA", "AMZN"]
    const periods = ["1d", "1wk", "1mo"]
    const availableTickers = await HDYFUtil.getAvailableYfTickersInDb()
    ...
    if (tickers.length > 0) {
        ...
        res.status(200).json({ message: "Yfinance tickers already available" });
    }
}
    ...
    try {
        ...
        for (const ticker of yfTickers) {
            ...
            if (daysElapsed > 1) {
                ...
                console.log("New data lendth", updateCount)
                if (updateCount > 0) {
