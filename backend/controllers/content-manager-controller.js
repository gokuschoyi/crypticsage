...
const getSections = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Sections fetch failed, db error" });
    } finally {
        close("getSections");
    }
}
...
const addSection = async (req, res) => {
    ...
    if (!title || !content) {
        return res.status(400).json({ message: "Invalid section data. (Parameters missing, title or content)" });
    } else {
        try {
            ...
            console.log(err)
            res.status(500).json({ message: "Section creation failed, db error" });
        } finally {
            close("addSection");
        }
    }
...
const updateSection = async (req, res) => {
    ...
    if (sectionId === '') {
        ...
        try {
            ...
            res.status(500).json({ message: "Section update failed, db error" });
        } finally {
            close("updateSection");
        }
    }
...
const deleteSection = async (req, res) => {
    ...
    if (sectionId === '') {
        ...
        try {
            ...
            res.status(500).json({ message: "Section deletion failed, db error" });
        } finally {
            close("deleteSection");
        }
    }
...
const getLessons = async (req, res) => {
    ...
    if (sectionId === '') {
        ...
        try {
            ...
            res.status(500).json({ message: "Lessons fetch failed, db error" });
        } finally {
            close("getLessons");
        }
    }
...
const addLesson = async (req, res) => {
    ...
    if (!chapter_title || !sectionId || !lessonData) {
        return res.status(400).json({ message: "Invalid or No parameters passed" });
    } else {
        try {
            let data = {
                chapter_title,
            ...
            res.status(500).json({ message: "Lesson creation failed, db error" });
        } finally {
            close("addLesson");
        }
    }
...
const updateLesson = async (req, res) => {
    const { chapter_title, lessonData, lessonId } = req.body;
    if (!lessonId) {
        return res.status(400).json({ message: "Section Id is required" });
    } else {
        try {
            ...
            if (lesson === null) {
                ...
                await lessonsCollection.updateOne({ lessonId }, { $set: { chapter_title, lessonData } });
                res.status(200).json({ message: "Lesson updated successfully" });
            }
        } catch (err) {
            res.status(500).json({ message: "Lesson update failed, db error" });
        } finally {
            close("updateLesson")
        }
    }
...
const deleteLesson = async (req, res) => {
    ...
    if (!lessonId) {
        ...
        try {
            ...
            res.status(500).json({ message: "Lesson deletion failed, db error" });
        } finally {
            close("deleteLesson");
        }
    }
...
const getQuizQuestions = async (req, res) => {
    ...
    if (!lessonId) {
        ...
        try {
            ...
            res.status(500).json({ message: "Quiz questions fetch failed, db error" });
        } finally {
...
const addQuizQuestions = async (req, res) => {
    ...
    if (!quizData) {
        ...
        try {
            ...
            res.status(500).json({ message: "Quiz question creation failed, db error" });
        } finally {
    ...
    if (!quizId) {
        ...
        try {
            ...
            if (quiz === null) {
                ...
                res.status(400).json({ message: "Quiz question not found" });
            } else {
            ...
            res.status(500).json({ message: "Quiz question update failed, db error" });
        }
    }
}
...
const deleteQuizQuestion = async (req, res) => {
    ...
    if (!quizId || !lessonId || !sectionId) {
        ...
        try {
            ...
            const quizCollection = db.collection('quiz');
            let delRes = await quizCollection.deleteOne({ quizId: quizId });
            console.log(delRes)
            let resuw = []
            if (delRes.deletedCount > 0) {
                ...
                res.status(400).json({message:'no quiz found with that id'})
            }
        } catch (err) {
            res.status(500).json({ message: "Quiz question deletion failed, db error" });
        } finally {
