...
const { periodToMilliseconds } = require('./cryptoHelper')
...
const handleTokenRedisStorage = async (req, res, next) => {
    ...
    if (!isTokenDataAvailableInRedis) {
        ...
        console.log("Data available in Redic Store. Skipping MongoDB fetch (Redis Handler) ")
    }
    next();
}
...
const fetchTokenData = async (dataSource, tokenName, period) => {
    ...
    try {
        ...
        return tokenData?.data[period] || ["no data"];
    } catch (err) {
        console.log(err);
    } finally {
